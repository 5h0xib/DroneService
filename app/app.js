...
myApp.service('DataService', ['$http', function ($http) {
    // Beverages
    this.getHomeMade = function () {
        return $http.get('data/organs.json');
    };

    // Food
    this.getBreakFast = function () {
        return $http.get('data/medicines.json');
    };
...
myApp.controller('AppController', ['$scope', 'DataService', 'FavoritesService', function ($scope, DataService, FavoritesService) {
    ...
    // Load all categories
    // Food
    loadCategory(DataService.getBreakFast, 'BreakFast');

    // Beverages
    loadCategory(DataService.getHomeMade, 'HomeMade');
    ...
    // Get Filter Items
    $scope.getFilteredItems = function (categoryData) {
        return categoryData?.filter(function (item) {
            return !$scope.search || item.name.toLowerCase().includes($scope.search.toLowerCase());
        }) || [];
    };    
